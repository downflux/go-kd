...
// position.
func Element(data PointSlice, n int) {
	l := data.Len()
	if n < 0 || n >= l {
		return
	}
...
// workloads.
func quickSelectAdaptive(data PointSlice, k, a, b int) {
	var (
...
func hoarePartition(data PointSlice, p, begin, end int) int {
...
func repeatedStepLeft(data PointSlice, k, a, b int) int {
	l := b - a  // |A|
	if l < 12 { // |A| < 12
...
func repeatedStepFarLeft(data PointSlice, k, a, b int) int {
	l := b - a  // |A|
	if l < 12 { // |A| < 12
		return hoarePartition(data, a+l/2, a, b) // HoarePartition(A, |A| / 2)
	}
...
// values data[a,b,c,d]
func lowerMedian4(data PointSlice, a, b, c, d int) {
...
func median3(data PointSlice, a, b, c int) {
	if data.Less(b, a) {
		data.Swap(b, a)
	}
	if data.Less(c, b) {
...
func quickSelect(partition func(data PointSlice, k, a, b int) int, data PointSlice, k, a, b int) {
	for {
...
// between A[:a] and A[b:].
func expandPartition(data PointSlice, a, p, b, begin, end int) int {
...
func simplePartition(data PointSlice, k, a, b int) int {
	p := a
	for i := a + 1; i < b; i++ {
		if data.Less(i, p) {
			data.Swap(p, i)
